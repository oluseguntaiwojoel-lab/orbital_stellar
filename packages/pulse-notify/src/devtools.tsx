...
export function registerConnection(info: Omit<DevConnection, "id" | "lastEvent"> & { id?: string }) {
  const id = info.id ?? `${info.serverUrl}::${info.address}::${Math.random().toString(36).slice(2,9)}`;
...
export function updateConnection(id: string, patch: Partial<Omit<DevConnection, "id" | "serverUrl" | "address" | "url"> & { lastEvent?: number }>) {
...
export function PulseNotifyDevtools(): JSX.Element | null {
  ...
  return (
    <div style={{ padding: 12, fontFamily: "Inter, system-ui, sans-serif", fontSize: 12 }}>
      ...
      <div style={{ maxHeight: 360, overflow: "auto", border: "1px solid #eee", padding: 8, borderRadius: 6 }}>
        ...
        {state.map((c) => (
          <div key={c.id} style={{ padding: 8, borderBottom: "1px dashed #f0f0f0" }}>
            ...
            <div><strong>Connected:</strong> {c.connected ? "yes" : "no"} {c.error ? ` — ${c.error}` : ""}</div>
            <div><strong>Last event:</strong> {c.lastEvent ? new Date(c.lastEvent).toLocaleString() : "—"}</div>
          </div>
        ))}
      </div>
    </div>
